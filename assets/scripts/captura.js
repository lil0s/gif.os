...
listen(".comenzar","click", () => {comenzar()} );
listen(".repetir", "click",() => {comenzar()} );
//iniciar captura
...
listen(".stop-video", "click", () => {stopGif() } );
...
async function uploadGif(){
    ...
    .then(async function(data)
    {
        ...
        //VEO EL ID DEL GIF QUE SUB
        gifUrl = await setStorage(data.data.id); 
        // veo el preview del gif que hice
...
listen('#copy-link', 'click', ()=>{copiaPortapapeles()});
...
listen('#download-file', 'click', ()=>{ download() });
...
listen("#videoControl", "click", () => {videoControl()} );
